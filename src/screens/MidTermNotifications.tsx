...
interface ApiExamItem {
  exam_date: string;                      // "2026-02-02"
  day_name: string;                       // "Monday"
  subject_name: string;                   // "Mathematics"
  exam_type: string;                      // "Mid Term"
  start_time: string;                     // "07:53:13"
  end_time: string;                       // "10:53:13"
...
interface MidTermNotificationsProps {
  navigation?: any;
  onBack?: () => void;
  institutionId?: number;   // allow caller to pass institution_id; defaults to -1
}
...
/**
 * Map an array of API items to internal ExamData, assigning stable colours per subject.
 */
...
// ─────────────────────────────────────────────
// Static / sample data kept for Logs & Staff
// ─────────────────────────────────────────────
const sampleLogs: LogEntry[] = [
  { id: '1', timestamp: 'Sep 20, 2023 - 10:30 AM', message: 'Notification sent to 285 parents', status: 'success' },
  { id: '2', timestamp: 'Sep 20, 2023 - 10:31 AM', message: 'Failed to send to 15 contacts',     status: 'failed' },
  { id: '3', timestamp: 'Sep 20, 2023 - 10:35 AM', message: 'Retry successful for 12 contacts',  status: 'success' },
];

const sampleStaff: StaffMember[] = [
  { id: '1', name: 'Dr. Alice Johnson',    email: 'alice.j@school.edu',  phone: '+1 234-567-8901', assignedExams: ['1','3'] },
  { id: '2', name: 'Prof. Bob Smith',      email: 'bob.s@school.edu',   phone: '+1 234-567-8902', assignedExams: ['2','4'] },
  { id: '3', name: 'Dr. Carol Williams',   email: 'carol.w@school.edu', phone: '+1 234-567-8903', assignedExams: ['1','2'] },
];
...
// Component
// ─────────────────────────────────────────────
const MidTermNotifications: React.FC<MidTermNotificationsProps> = ({
  navigation,
  onBack,
  institutionId = -1,
}) => {
  ...
  // ─────────────────────────────────────────────
  // Fetch mid-term exams from API
  // ─────────────────────────────────────────────
  const fetchMidTermExams = useCallback(async () => {
    setLoading(true);
    setFetchError(null);
    ...
    try {
      ...
      // Extra safety: keep only items whose exam_type is exactly "Mid Term"
      const midTermOnly = data.filter(
        (item) => item.exam_type?.toLowerCase() === 'mid term'
      );

      setExamData(mapApiToExams(midTermOnly));
    } catch (err: any) {
      setFetchError(err.message || 'Failed to fetch exam schedule');
      Alert.alert('Fetch Error', err.message || 'Unable to load exam schedule. Please try again.');
    } finally {
      setLoading(false);
    }
  }, [institutionId]);
  ...
  // Derived data
  // ─────────────────────────────────────────────
  /** Group exams by "Day, Date" for the timetable */
  ...
  const handleBack = () => {
    if (currentScreen === 'timetable') {
      ...
      else                 Alert.alert('Navigation', 'Please provide navigation prop or onBack callback');
    } else {
      setCurrentScreen('timetable');
      setSelectedExam(null);
    }
  ...
  const handleExportPDF = async () => {
    try {
      ...
      await Share.share({ message, title: 'Exam Timetable' });
    } catch {
  ...
  const handleDeleteReminder = (id: string) => {
    Alert.alert('Delete Reminder', 'Are you sure you want to delete this reminder?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Delete',
        style: 'destructive',
        onPress: () => setSavedReminders((prev) => prev.filter((r) => r.id !== id)),
      },
    ]);
  };
            ...
            <View key={log.id} style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
              <View style={styles.logItem}>
                <View style={[styles.logIcon,
                  ...
                  <MaterialIcons name={log.status === 'success' ? 'check-circle' : 'error'} size={24}
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.headerWithBack}>
        ...
        <View style={styles.headerCenterContent}>
          <Text style={[styles.headerTitleCenter, { color: colors.text }]}>Exam Timetable</Text>
        </View>
        <View style={styles.headerSpacer} />
      </View>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {loading && (
          <View style={styles.loadingContainer}>
            ...
            <Text style={[styles.loadingText, { color: colors.subText }]}>Fetching Mid-Term Schedule…</Text>
          </View>
        )}
              ...
              <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
                <View style={styles.notificationCard}>
                  ...
                  <View style={styles.notificationContent}>
                    <Text style={[styles.notificationTitle, { color: colors.text }]}>Mid-term Notifications</Text>
                  ...
                  {exams.map((exam, index) => (
                    <TouchableOpacity key={exam.id}
                      style={[styles.examItem,
                        index < exams.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.border }]}
            ...
            <View style={styles.section}>
              ...
              <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
                <View style={styles.staffContainer}>
                  ...
                  <Text style={[styles.staffText, { color: colors.subText }]}>15 Staff Assigned</Text>
                </View>
                <TouchableOpacity
  ...
  const renderReminderScreen = () => (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.surface }]}>
      ...
      <View style={[styles.reminderHeader, { backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.reminderTitleCenter, { color: colors.text }]}>Set Exam Reminder</Text>
        <View style={styles.headerSpacer} />
      </View>
      ...
      <ScrollView style={styles.reminderScrollView} contentContainerStyle={styles.reminderScrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.reminderSection}>
          ...
          <View style={styles.inputContainer}>
            <Text style={[styles.inputLabel, { color: colors.text }]}>Exam Subject</Text>
          ...
          <View style={[styles.settingsCard, { backgroundColor: colors.card }]}>
            ...
            <View style={[styles.settingItem, { borderBottomColor: colors.border }]}>
              ...
              <Switch value={reminderEnabled} onValueChange={setReminderEnabled}
                ...
                thumbColor="#FFFFFF" ios_backgroundColor={colors.surfaceAlt} />
            </View>
      ...
      <View style={[styles.reminderFooter, { backgroundColor: colors.surface, borderTopColor: colors.border }]}>
        <TouchableOpacity style={[styles.saveButton, { backgroundColor: colors.primary }]}
          onPress={handleSaveReminder} activeOpacity={0.8}>
...
const styles = StyleSheet.create({
  ...
  headerCenterContent: { flex: 1, alignItems: 'center' },
  headerSpacer: { width: 36 },
  backButton: { flexDirection: 'row', alignItems: 'center' },
  backText: { fontSize: 16, fontWeight: '500', marginLeft: 4 },
  headerTitle: { fontSize: 18, fontWeight: '700' },
  headerTitleCenter: { fontSize: 20, fontWeight: '700' },
  ...
  card: {
    ...
    shadowOpacity: 0.05, shadowRadius: 4, elevation: 2, marginBottom: 12,
  },
  ...
  iconContainer: {
    ...
    backgroundColor: '#F59E0B1A', justifyContent: 'center', alignItems: 'center', marginRight: 16,
  },
  notificationContent: { flex: 1 },
  notificationTitle: { fontSize: 16, fontWeight: '700', marginBottom: 4 },
  notificationSubtitle: { fontSize: 14 },
  statusBadge: { backgroundColor: '#10B9811A', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 12 },
  statusBadgeText: { color: '#10B981', fontSize: 10, fontWeight: '700' },
  notificationFooter: {
  ...
  examDetails: { flex: 1 },
  examSubject: { fontSize: 14, fontWeight: '700', marginBottom: 4 },
  ...
  avatar: {
    ...
    justifyContent: 'center', alignItems: 'center', marginLeft: -8, borderWidth: 2,
  },
  ...
  homeIndicator: {
    ...
    width: 128, height: 4, borderRadius: 2, transform: [{ translateX: -64 }],
  },
  ...
  settingItem: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
    paddingHorizontal: 16, paddingVertical: 14, minHeight: 56, borderBottomWidth: 1,
  },
  ...
  infoBoxContent: { paddingHorizontal: 16, paddingVertical: 16, borderRadius: 12, borderWidth: 1 },
  infoText: { fontSize: 14, fontStyle: 'italic', textAlign: 'center', lineHeight: 20 },
